...
sed -i '/uci commit system/i\uci set system.@system[0].hostname='OpenWrt-Xmyr'' package/lean/default-settings/files/zzz-default-settings

# 版本号里显示一个自己的名字（281677160 build $(TZ=UTC-8 date "+%Y.%m.%d") @ 这些都是后增加的）
sed -i "s/OpenWrt /Xmyr build $(TZ=UTC-8 date "+%Y.%m.%d") @ OpenWrt /g" package/lean/default-settings/files/zzz-default-settings
...
sed -i 's/"管理权"/"管理权"/g' feeds/luci/modules/luci-base/po/zh-cn/base.po
